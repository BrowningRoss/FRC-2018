...
public class RevDigitBoard {
    ...
    private final DigitalInput mButtonA, mButtonB;
    private final AnalogInput mPot;

    private byte[][] charreg;
    Map charmap;

    public RevDigitBoard() {
        mI2C = new I2C(Port.kMXP, 0x70);
        mButtonA = new DigitalInput(19);
        mButtonB = new DigitalInput(20);
        mPot = new AnalogInput(3);
        ...
        mI2C.writeBulk(osc);
        Timer.delay(.01);
        mI2C.writeBulk(bright);
        Timer.delay(.01);
        mI2C.writeBulk(blink);
        Timer.delay(.01);

        charreg = new byte[37][2];  //character registry
