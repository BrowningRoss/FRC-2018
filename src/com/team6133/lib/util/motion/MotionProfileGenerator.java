package com.team6133.lib.util.motion;

import com.team6133.lib.util.motion.MotionProfileGoal.CompletionBehavior;
...
public class MotionProfileGenerator {
	...
	public synchronized static MotionProfile generateProfile(MotionProfileConstraints constraints,
		...
		if (delta_pos < 0.0 || (delta_pos == 0.0 && prev_state.vel() < 0.0)) {
			// For simplicity, we always assume the goal requires positive
			// movement. If negative, we flip to solve, then
			// flip the solution.
		...
		// If our velocity is headed away from the goal, the first thing we need
		// to do is to stop.
		...
		if (min_abs_vel_at_goal_sqr > 0.0
			...
			if (goal_state.completion_behavior() == CompletionBehavior.VIOLATE_MAX_ABS_VEL) {
				...
				if (Math.abs(delta_pos) < goal_state.pos_tolerance()) {
					// Special case: We are at the goal but moving too fast.
					// This requires 'infinite' acceleration,
					// which will result in NaNs below, so we can return the
					// profile immediately.
				...
				// We are going to overshoot the goal, so the first thing we
				// need to do is come to a stop.
				final double stopping_time = Math.abs(start_state.vel() / constraints.max_abs_acc());
				profile.appendControl(-constraints.max_abs_acc(), stopping_time);
				// Now we need to travel backwards, so generate a flipped
				// profile.
		...
		// Invariant from this point forward: We can achieve goal_vel at
		// goal_state.pos exactly using no more than +/-
		// max_acc.

		// What is the maximum velocity we can reach (Vmax)? This is the
		// intersection of two curves: one accelerating
		// towards the goal from profile.finalState(), the other coming from the
		// goal at max vel (in reverse). If Vmax
