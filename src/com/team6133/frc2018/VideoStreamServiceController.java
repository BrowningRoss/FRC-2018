 ...
 * restarted and if the webcam is unplugged and changes names.
 */
public class VideoStreamServiceController {
					...
					try {
						...
						for (WatchEvent<?> event : key.pollEvents()) {
							...
							if (!newFilename.getFileName().toString().startsWith("video")) {
								System.out.println("Don't care about new device: " + newFilename.toString());
								continue;
							}
	...
	private void restartProcess(Path newVideoDevice) {
		...
		try {
			mExistingProcess = new ProcessBuilder("/usr/local/bin/mjpg_streamer", "-i",
					"/usr/local/lib/mjpg-streamer/input_uvc.so --device " + newVideoDevice.toString() + " -r 160x120",
					"-o", "/usr/local/lib/mjpg-streamer/output_http.so -w /usr/local/share/mjpg-streamer/www -p 5801")
