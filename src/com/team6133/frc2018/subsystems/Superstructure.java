...
import com.team6133.frc2018.Robot;
...
public class Superstructure extends Subsystem {
	...
	// ~!@private final Solenoid mHopperSolenoid =
	// Constants.makeSolenoidForId(Constants.kHopperSolenoidId);
	...
	public enum SystemState {
		IDLE, WAITING_FOR_ALIGNMENT, // waiting for the drivebase to aim
		WAITING_FOR_FLYWHEEL, // waiting for the shooter to spin up
		SHOOTING, // shooting
		SHOOTING_SPIN_DOWN, // short period after the driver releases the shoot
							// button where the flywheel
							// continues to spin so the last couple of shots
							// don't go short
		...
		RANGE_FINDING // blink the LED strip to let drivers know if they are at
						// an optimal shooting range
	};
	...
	private SystemState handleShooting(double timestamp) {
		...
		if ((timestamp - mCurrentStateStartTime < Constants.kShooterMinShootingTime) || !mShooterRpmBuffer.isFull()
	...
	public synchronized boolean autoSpinShooter(boolean allow_shooting) {
		...
		final Optional<ShooterAimingParameters> aimOptional = RobotState.getInstance().getAimingParameters();
		mLED.setWantedState(LED.WantedState.FIND_RANGE);
		if (aimOptional.isPresent()) {
			...
			if (mShooter.getSetpointRpm() < Constants.kShooterTuningRpmFloor) {
				// Hold setpoint if we were already spinning, since it's our
				// best guess as to the range once the goal
				// re-appears.
