...
//~!@import com.team6133.lib.util.drivers.MB1043;
...
public class MotorGearGrabber extends Subsystem {
	...
	public enum WantedState {
		IDLE, ACQUIRE, SCORE, CLEAR_BALLS
	}
	...
	private enum SystemState {
		...
		STOWING, // transition state between INTAKE and STOWED where the motor
					// intakes so the gear doesn't fly out as the subsystem
					// pivots up
	...
	private MotorGearGrabber() {
		// ~!@mWristSolenoid =
		// Constants.makeSolenoidForId(Constants.kGearWristSolenoid);
		// ~!@mMasterTalon =
		// CANTalonFactory.createDefaultTalon(Constants.kGearGrabberId);
	...
	public void outputToSmartDashboard() {
		// ~!@SmartDashboard.putNumber("Gear Grabber Current",
		// mMasterTalon.getOutputCurrent());
	}
	...
	private SystemState handleIntake(double timeInState) {
		...
		default:
			...
			// check if the current has been above a threshold value for enough
			// time.
	...
	/*
	 * ~!@ private void setWristUp() { mWristUp = true;
	 * mWristSolenoid.set(mWristUp); }
	 * 
	 * private void setWristDown() { mWristUp = false;
	 * mWristSolenoid.set(mWristUp); }
	 */
