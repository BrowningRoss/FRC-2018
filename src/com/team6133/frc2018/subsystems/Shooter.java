...
import com.team6133.frc2018.ShooterAimingParameters;
import com.team6133.frc2018.loops.Loop;
import com.team6133.frc2018.loops.Looper;
import com.team6133.lib.util.CircularBuffer;
...
/**
 * The shooter subsystem consists of 4 775 Pro motors driving twin backspin
 * flywheels. When run in reverse, these motors power the robot's climber
 * through a 1 way bearing. The shooter subsystem goes through 3 stages when
 * shooting. 1. Spin Up Use a PIDF controller to spin up to the desired RPM. We
 * acquire this desired RPM by converting the camera's range value into an RPM
 * value using the range map in the {@link Constants} class. 2. Hold When Ready
 * Once the flywheel's RPM stabilizes (remains within a certain bandwidth for
 * certain amount of time), the shooter switches to the hold when ready stage.
 * In this stage, we collect kF samples. The idea is that we want to run the
 * shooter in open loop when we start firing, so in this stage we calculate the
 * voltage we need to supply to spin the flywheel at the desired RPM. 3. Hold
 * Once we collect enough kF samples, the shooter switches to the hold stage.
 * This is the stage that we begin firing balls. We set kP, kI, and kD all to 0
 * and use the kF value we calculated in the previous stage for essentially open
 * loop control. The reason we fire in open loop is that we found it creates a
 * much narrower stream and leads to smaller RPM drops between fuel shots.
 * 
 * @see Subsystem.java
 */
public class Shooter extends Subsystem {
	...
	// ~!@private final CANTalon mRightMaster, mRightSlave, mLeftSlave1,
	// mLeftSlave2;
	...
	private Shooter() {
		/*
		 * ~!@ mRightMaster =
		 * CANTalonFactory.createDefaultTalon(Constants.kRightShooterMasterId);
		 * mRightMaster.changeControlMode(CANTalon.TalonControlMode.Voltage);
		 * mRightMaster.setFeedbackDevice(CANTalon.FeedbackDevice.
		 * CtreMagEncoder_Relative); mRightMaster.reverseSensor(true);
		 * mRightMaster.reverseOutput(false);
		 * mRightMaster.enableBrakeMode(false);
		 * mRightMaster.SetVelocityMeasurementPeriod(CANTalon.
		 * VelocityMeasurementPeriod.Period_10Ms);
		 * mRightMaster.SetVelocityMeasurementWindow(32);
		 * mRightMaster.setNominalClosedLoopVoltage(12);
		 * 
		 * mRightMaster.setStatusFrameRateMs(CANTalon.StatusFrameRate.General,
		 * 2); mRightMaster.setStatusFrameRateMs(CANTalon.StatusFrameRate.
		 * AnalogTempVbat, 2);
		 * 
		 * CANTalon.FeedbackDeviceStatus sensorPresent = mRightMaster
		 * .isSensorPresent(CANTalon.FeedbackDevice.CtreMagEncoder_Relative); if
		 * (sensorPresent !=
		 * CANTalon.FeedbackDeviceStatus.FeedbackStatusPresent) {
		 * DriverStation.reportError("Could not detect shooter encoder: " +
		 * sensorPresent, false); }
		 * 
		 ...
		 * 
		 * System.out.println("RPM Polynomial: " +
		 * Constants.kFlywheelAutoAimPolynomial);
		 * 
		 * mCSVWriter = new ReflectingCSVWriter<ShooterDebugOutput>(
		 * "/home/lvuser/SHOOTER-LOGS.csv", ShooterDebugOutput.class);
		 */
	}
	...
	public void refreshControllerConsts() {
		 ...
		 * 
		 * mRightMaster.setProfile(kHoldProfile); mRightMaster.setP(0.0);
		 * mRightMaster.setI(0.0); mRightMaster.setD(0.0);
		 * mRightMaster.setF(Constants.kShooterTalonKF);
	...
	public synchronized void outputToSmartDashboard() {
		 ...
		 * SmartDashboard.putNumber("shooter_speed_error", mSetpointRpm -
		 * current_rpm); SmartDashboard.putNumber("shooter_output_voltage",
		 * mRightMaster.getOutputVoltage());
		 * SmartDashboard.putNumber("shooter_setpoint", mSetpointRpm);
		 * 
		 * SmartDashboard.putBoolean("shooter on target", isOnTarget());
		 */
		// SmartDashboard.putNumber("shooter_talon_position",
		// mRightMaster.getPosition());
		// SmartDashboard.putNumber("shooter_talon_enc_position",
		// mRightMaster.getEncPosition());
	}
	...
	/**
	 * Main control loop of the shooter. This method will progress the shooter
	 * through the spin up, hold when ready, and hold stages.
	 */
	private void handleClosedLoop(double timestamp) {
		...
		if (mControlMethod == ControlMethod.HOLD) {
			// Update Kv if we exceed our target velocity. As the system heats
			// up, drag is reduced.
	...
	private double getSpeedRpm() {
		return 0.0;// ~!@mRightMaster.getSpeed();
	}

	private static CANTalon makeSlave(int talonId, boolean flipOutput) {
		CANTalon slave = CANTalonFactory.createPermanentSlaveTalon(talonId, Constants.kRightDriveMasterId);// ~!@CHANGE
																											// to
																											// kRightShooterMasterId
	...
	public boolean checkSystem() {
		 ...
		 * 
		 * mRightMaster.set(6.0f); Timer.delay(4.0); final double
		 * currentRightMaster = mRightMaster.getOutputCurrent(); final double
		 * rpmMaster = mRightMaster.getSpeed(); mRightMaster.set(0.0f);
		 * 
		 * Timer.delay(2.0);
		 * 
		 * mRightSlave.set(6.0f); Timer.delay(4.0); final double
		 * currentRightSlave = mRightSlave.getOutputCurrent(); final double
		 * rpmRightSlave = mRightMaster.getSpeed(); mRightSlave.set(0.0f);
		 * 
		 * Timer.delay(2.0);
		 * 
		 * mLeftSlave1.set(-6.0f); Timer.delay(4.0); final double
		 * currentLeftSlave1 = mLeftSlave1.getOutputCurrent(); final double
		 * rpmLeftSlave1 = mRightMaster.getSpeed(); mLeftSlave1.set(0.0f);
		 * 
		 * Timer.delay(2.0);
		 * 
		 * mLeftSlave2.set(-6.0f); Timer.delay(4.0); final double
		 * currentLeftSlave2 = mLeftSlave2.getOutputCurrent(); final double
		 * rpmLeftSlave2 = mRightMaster.getSpeed(); mLeftSlave2.set(0.0f);
		 * 
		 ...
		 * 
		 * System.out.println("Shooter Right Master Current: " +
		 * currentRightMaster + " Shooter Right Slave Current: " +
		 * currentRightSlave);
		 * System.out.println("Shooter Left Slave One Current: " +
		 * currentLeftSlave1 + " Shooter Left Slave Two Current: " +
		 * currentLeftSlave2); System.out.println("Shooter RPM Master: " +
		 * rpmMaster + " RPM Right slave: " + rpmRightSlave +
		 * " RPM Left Slave 1: " + rpmLeftSlave1 + " RPM Left Slave 2: " +
		 * rpmLeftSlave2);
		 */
		boolean failure = false;
		/*
		 * ~!@ if (currentRightMaster < kCurrentThres) { failure = true;
		 * System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter Right Master Current Low !!!!!!!!!!"
		 * ); }
		 * 
		 * if (currentRightSlave < kCurrentThres) { failure = true; System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter Right Slave Current Low !!!!!!!!!!"
		 * ); }
		 * 
		 * if (currentLeftSlave1 < kCurrentThres) { failure = true; System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter Left Slave One Current Low !!!!!!!!!!"
		 * ); }
		 * 
		 * if (currentLeftSlave2 < kCurrentThres) { failure = true; System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter Left Slave Two Current Low !!!!!!!!!!"
		 * ); }
		 * 
		 * if (!Util.allCloseTo(Arrays.asList(currentRightMaster,
		 * currentRightSlave, currentLeftSlave1, currentLeftSlave2),
		 * currentRightMaster, 5.0)) { failure = true; System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter currents different !!!!!!!!!!!!!!!!!"
		 * ); }
		 * 
		 * if (rpmMaster < kRpmThres) { failure = true; System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter Master RPM Low !!!!!!!!!!!!!!!!!!!!!!!"
		 * ); }
		 * 
		 * if (rpmRightSlave < kRpmThres) { failure = true; System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter Right Slave RPM Low !!!!!!!!!!!!!!!!!!!!!!!"
		 * ); }
		 * 
		 * if (rpmLeftSlave1 < kRpmThres) { failure = true; System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter Left Slave1 RPM Low !!!!!!!!!!!!!!!!!!!!!!!"
		 * ); }
		 * 
		 * if (rpmLeftSlave2 < kRpmThres) { failure = true; System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter Left Slave2 RPM Low !!!!!!!!!!!!!!!!!!!!!!!"
		 * ); }
		 * 
		 * if (!Util.allCloseTo(Arrays.asList(rpmMaster, rpmRightSlave,
		 * rpmLeftSlave1, rpmLeftSlave2), rpmMaster, 250)) { failure = true;
		 * System.out.
		 * println("!!!!!!!!!!!!!!!!!! Shooter RPMs different !!!!!!!!!!!!!!!!!!!!!!!"
		 * ); }
		 */
		return !failure;
	}
